...
public class SignatureFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		//BYPASS SERVICES START
//		List<String> listService = new ArrayList<String>();
//		listService.add("/etb/getProductList");
//		listService.add("/etb/getProductDetail");
		//BYPASS SERVICES END
		
		RequestWrapper req = new RequestWrapper((HttpServletRequest) request);
//		String path = req.getRequestURI().substring(req.getContextPath().length());
		
//		if(listService!=null && listService.size() > 0 && listService.stream().anyMatch(service -> path.equals(service))) {
